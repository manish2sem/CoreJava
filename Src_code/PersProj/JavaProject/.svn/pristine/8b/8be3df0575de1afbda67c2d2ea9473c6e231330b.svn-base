package CoreConcepts.connectionPool;
...
public class ConnectionPool implements Runnable {
	...
	public synchronized Connection getConnection() throws SQLException {
		if (!availableConnections.isEmpty()) {
	       ...
	       1) You haven't reached maxConnections limit. 
	       So establish one in the background if there isn't already one pending, 
	       then wait for the next available connection (whether or not it was the newly established one).
	       2) You reached maxConnections limit and waitIfBusy flag is false. Throw SQLException in such a case.
	       3) You reached maxConnections limit and waitIfBusy flag is true. 
	...
	// This explicitly makes a new connection. while initializing the ConnectionPool,
	 ...
	 *  Note that you are not <I>required to call this when done with a ConnectionPool, 
	 *  since connections are guaranteed to be closed when garbage collected. 
	 *  But this method gives more control regarding when the connections are closed.
	 */
